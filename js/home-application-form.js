    ...
    function initMultiStepForm() {
        ...
        async function submitApplication() {
            ...
            const applicationData = {
                ...
                clientPhone: data.cell_phone,
                clientSsn: data.ssn || 'PENDING',
                clientAddress: data.street_address,
                clientCity: data.city,
                clientState: data.state,
                clientZipCode: data.zip_code,
                ...
                // Credit request
                desiredAmount: parseFloat(data.desired_amount) || 0,
                purpose: data.purpose || 'Personal loan',
                
                // Partner (default - will be assigned by admin)
                partnerId: 'pending-partner-assignment',
            ...
            console.log('📤 Sending to API:', applicationData);
            ...
            try {
                ...
                // Show success message with real application ID
                showSuccessMessage(result.data?.id || result.id);
        ...
        function showSuccessMessage(applicationId) {
            const referenceNumber = applicationId ? applicationId.slice(0, 8).toUpperCase() : Date.now().toString().slice(-8);
        ...
        function showErrorMessage(errorMsg) {
            const errorHTML = `
                <div style="background: #f8d7da; border: 2px solid #dc3545; border-radius: 8px; padding: 30px; text-align: center; margin: 20px 0;">
                    ...
                    <p style="color: #721c24; margin-bottom: 20px;">
                        We encountered an error while submitting your application. <br>
                        ${errorMsg || 'Please try again or contact our support team.'}
                    </p>
                    <button onclick="location.reload()" class="btn btn-danger" style="margin-top: 20px;">
