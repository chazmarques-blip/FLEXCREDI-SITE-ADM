...
document.addEventListener('DOMContentLoaded', function() {
    console.log('FLEXCREDI Dashboard - Sistema carregado');
...
function loadUserData() {
    ...
    if (!userData || !userData.preApproval) {
        // Create test user for demonstration or update existing user
...
function updateUserInterface(userData) {
    ...
    if (userGreeting) {
        userGreeting.textContent = `Bem-vindo, ${userData.fullName || 'Cliente'}!`;
    }
    
    if (userName) {
        userName.textContent = `Bem-vindo, ${userData.fullName || 'Cliente'}`;
    }
    ...
    if (userAvatar) {
        ...
        userAvatar.alt = `Avatar de ${userData.fullName || 'Cliente'}`;
    }
    ...
    if (userData && userData.recentActivity) {
        ...
        activities = [
            ...
            },
            {
                ...
                description: 'Documentos recebidos e em análise',
                time: '5 dias atrás'
            },
            {
...
function updateContractInformation(userData) {
    ...
    if (isFinallyApproved && userData.finalApproval.amount) {
        ...
        if (contractStatus) contractStatus.textContent = 'Aprovado - Pronto para Assinatura';
        if (contractTitle) contractTitle.textContent = `Contrato de Empréstimo #${contractData.contractNumber}`;
    } else if (isPreApproved && userData.preApproval) {
        // Usar dados da pré-aprovação
        contractData = userData.preApproval;
        if (contractStatus) contractStatus.textContent = 'Pré-Aprovado - Sujeito a Análise Final';
        if (contractTitle) contractTitle.textContent = `Pré-Contrato #${contractData.contractNumber}`;
    } else {
        // Estado padrão
        return;
    }
    ...
    if (contractValueElements.length >= 4) {
        ...
        contractValueElements[2].textContent = `${contractData.termMonths} meses`;
        contractValueElements[3].textContent = `${formatCurrency(contractData.monthlyPayment)}/mês`;
    }
    
    // Atualizar botões baseado no status
    if (isPreApproved) {
        if (reviewContractBtn) {
            reviewContractBtn.innerHTML = '<i class="fas fa-eye"></i> Revisar Pré-Aprovação';
        }
        if (signContractBtn) {
            signContractBtn.innerHTML = '<i class="fas fa-clock"></i> Aguardando Documentos';
            signContractBtn.disabled = true;
            signContractBtn.style.opacity = '0.6';
        }
    } else if (isFinallyApproved) {
        if (reviewContractBtn) {
            reviewContractBtn.innerHTML = '<i class="fas fa-eye"></i> Revisar Contrato Final';
        }
        if (signContractBtn) {
            signContractBtn.innerHTML = '<i class="fas fa-signature"></i> Assinar Eletronicamente';
...
function updateFinancingDetailsInHeader(userData) {
    ...
    if (userData.status === 'approved' && userData.finalApproval && userData.finalApproval.amount) {
        ...
        statusText = 'APROVADO';
        descriptionText = 'Pronto para assinatura do contrato';
        if (statusBadge) {
        ...
        descriptionText = 'Sua pré-aprovação está em fase de análise, submeta a documentação necessária para finalizar o processo.';
        if (statusBadge) {
        ...
        descriptionText = 'Aguardando análise da documentação';
        if (statusBadge) {
    ...
    if (financingTerm) {
        financingTerm.textContent = `${financingData.termMonths} meses`;
    }
    
    if (financingPayment) {
        financingPayment.textContent = `${formatCurrency(financingData.monthlyPayment)}/mês`;
    }
...
function loadProfileSection(userData) {
    ...
    const profileData = [
        { label: 'Nome Completo', field: 'fullName', value: userData.fullName || '', editable: true },
        { label: 'Email', field: 'email', value: userData.email || '', editable: true },
        { label: 'Telefone', field: 'phone', value: userData.phone || '', editable: true },
        { label: 'SSN', field: 'ssn', value: userData.ssn || '', editable: false },
        { label: 'Endereço', field: 'address', value: userData.address || '', editable: true },
        { label: 'CEP', field: 'zipCode', value: userData.zipCode || '', editable: true },
        { label: 'Cidade', field: 'city', value: userData.city || '', editable: true },
        { label: 'Estado', field: 'state', value: userData.state || '', editable: true }
    ];
...
function handleDocumentUpload(docType, file) {
    ...
    if (file.size > maxSize) {
        showAlert('error', 'Arquivo muito grande. Máximo 5MB permitido.');
        return;
    }
    
    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'application/pdf'];
    if (!allowedTypes.includes(file.type)) {
        showAlert('error', 'Tipo de arquivo não permitido. Use JPG, PNG ou PDF.');
        return;
    }
    ...
    previewBox.innerHTML = `
        ...
        <span class="preview-text">Enviando...</span>
    `;
    ...
    setTimeout(() => {
        ...
        showAlert('success', `${getDocumentName(docType)} enviado com sucesso!`);
...
function updateDocumentCompletion() {
    ...
    if (completionElement) {
        completionElement.textContent = `${approvedDocs}/${totalDocs} Enviados`;
    ...
    const modalHTML = `
        <div class="modal-overlay" id="signature-modal">
            <div class="modal-content signature-modal">
                ...
                <div class="modal-footer">
                    <button class="btn btn-outline" onclick="closeModal('signature-modal')">Cancelar</button>
                    <button class="btn btn-primary" id="nextStepBtn" onclick="nextSignatureStep()">Próximo</button>
                </div>
            </div>
        </div>
    `;
...
function nextSignatureStep() {
    ...
    if (nextStep && nextStep.classList.contains('step')) {
        ...
        if (nextStep.id === 'step-3') {
            nextBtn.textContent = 'Finalizar Assinatura';
...
function completeSignature() {
    ...
    if (contractStatus) {
        contractStatus.textContent = 'Contrato Assinado';
    ...
    if (signBtn) {
        signBtn.innerHTML = '<i class="fas fa-check"></i> Assinado';
    ...
    if (applicationStatus) {
        applicationStatus.innerHTML = `
            ...
            <span class="status-description">Contrato assinado - Liberação em 24h</span>
        `;
    }
    
    showAlert('success', 'Contrato assinado com sucesso! Você receberá o dinheiro em até 24 horas úteis.');
}
    ...
    const modalHTML = `
        <div class="modal-overlay" id="cobuyer-modal">
            <div class="modal-content cobuyer-modal">
                        ...
                        <div class="cobuyer-form-grid">
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-input" name="relationship">
                                    ...
                                    <option value="business_partner">Parceiro de Negócio</option>
                ...
                <div class="modal-footer">
                    <button class="btn btn-outline" onclick="closeModal('cobuyer-modal')">Cancelar</button>
                    <button class="btn btn-primary" onclick="saveCobuyerForm()">
                        <i class="fas fa-save"></i> Salvar Co-signatário
                    </button>
                </div>
            </div>
        </div>
...
function saveCobuyerForm() {
    ...
    renderCobuyers();
    showAlert('success', 'Co-signatário adicionado com sucesso!');
}
        ...
        <div class="cobuyer-card" data-id="${cobuyer.id}">
            ...
            <div class="cobuyer-content">
                <div class="cobuyer-info-section">
                    <div class="cobuyer-section-title">Informações Pessoais</div>
                ...
                <div class="cobuyer-info-section">
                    <div class="cobuyer-section-title">Documentos</div>
                ...
                <div class="cobuyer-contract-section">
                    <div class="cobuyer-section-title">Contrato de Co-signatário</div>
                    <div class="cobuyer-contract-status">
                        <div class="contract-info">
                            <i class="fas fa-${cobuyer.contractSigned ? 'check-circle' : 'clock'}"></i>
                            <span>${cobuyer.contractSigned ? 'Contrato Assinado' : 'Aguardando Assinatura'}</span>
                        </div>
...
function renderCobuyerDocuments(cobuyer) {
    const documents = [
        { key: 'driverLicense', name: 'Carteira de Motorista', icon: 'id-card', desc: 'Foto frente e verso' },
        { key: 'proofAddress', name: 'Comprovante de Residência', icon: 'home', desc: 'Conta de luz/água (máx. 90 dias)' },
        { key: 'socialSecurity', name: 'Social Security Card', icon: 'id-badge', desc: 'Cartão do Social Security' },
        { key: 'bankCard', name: 'Cartão Bancário', icon: 'credit-card', desc: 'Para débito automático das parcelas' }
    ];
...
function getCobuyerStatusText(cobuyer) {
    if (cobuyer.contractSigned) return 'Completo';
    if (cobuyer.documentsComplete) return 'Aguardando Contrato';
...
function getRelationshipText(relationship) {
    const relationships = {
        ...
        'sibling': 'Irmão/Irmã',
        'friend': 'Amigo',
        'business_partner': 'Parceiro de Negócio',
        'other': 'Outro'
    };
...
function removeCobuyer(cobuyerId) {
    if (confirm('Tem certeza que deseja remover este co-signatário?')) {
        ...
        renderCobuyers();
        showAlert('success', 'Co-signatário removido com sucesso!');
    }
}
    ...
    const modalHTML = `
        <div class="modal-overlay" id="cobuyer-contract-modal">
            <div class="modal-content contract-modal">
                ...
                <div class="modal-footer">
                    <button class="btn btn-outline" onclick="closeModal('cobuyer-contract-modal')">Fechar</button>
                    <button class="btn btn-primary" onclick="closeModal('cobuyer-contract-modal'); signCobuyerContract('${cobuyerId}');">
                        <i class="fas fa-signature"></i> Prosseguir para Assinatura
                    </button>
                </div>
            </div>
        </div>
    ...
    const modalHTML = `
        <div class="modal-overlay" id="cobuyer-signature-modal">
            <div class="modal-content signature-modal">
                ...
                <div class="modal-footer">
                    <button class="btn btn-outline" onclick="closeModal('cobuyer-signature-modal')">Cancelar</button>
...
function getCobuyerButtonContent(cobuyer, docType) {
    if (cobuyer.documents[docType].uploaded) {
        return '<i class="fas fa-sync"></i> Substituir';
    } else {
...
function getCobuyerMainPreviewContent(cobuyer, docType, iconName) {
    ...
    if (docData.uploaded && docData.previewData) {
        ...
        return `
            ...
            <span class="preview-text">Pendente</span>
        `;
    }
}
...
function handleSignerDocumentUpload(docType, file) {
    ...
    setTimeout(() => {
        ...
        const userData = getUserFromStorage();
        if (userData) {
            userData.documents[docType].uploaded = true;
            userData.documents[docType].approved = true;
            userData.documents[docType].fileName = file.name;
            
            // Update preview box
            updateSignerPreviewBox(docType, file, previewBox);
...
function updateSignerPreviewBox(docType, file, previewBox) {
    if (!previewBox) return;
    ...
    if (file.type.startsWith('image/')) {
        const reader = new FileReader();
        reader.onload = function(e) {
            previewBox.innerHTML = `<img src="${e.target.result}" alt="${file.name}" onclick="previewSignerDocument('${docType}')">`;
        };
        reader.readAsDataURL(file);
    } else if (file.type === 'application/pdf') {
        // For PDF files, render first page as image
        renderSignerPDFAsImage(file, previewBox, docType);
    } else {
        // For other files, show icon with filename
        const iconMap = {
            'driverLicense': 'fa-id-card',
...
function updateMainSignerStatus(userData) {
    ...
    if (completionElement) {
        if (docsUploaded === totalDocs) {
            ...
            completionElement.textContent = `${docsUploaded}/${totalDocs} Documentos`;
    ...
    if (statusElement) {
        if (docsUploaded === totalDocs) {
            ...
            statusElement.innerHTML = '<i class="fas fa-exclamation-triangle"></i><span>Documentos Pendentes</span>';
        }
    }
}
            ...
            if (fieldName === 'relationship') {
                ...
                field.innerHTML = `
                    <select class="profile-edit-input" data-field="${fieldName}">
                        <option value="spouse" ${currentRelationship === 'spouse' ? 'selected' : ''}>Cônjuge</option>
                        <option value="parent" ${currentRelationship === 'parent' ? 'selected' : ''}>Pai/Mãe</option>
                        <option value="sibling" ${currentRelationship === 'sibling' ? 'selected' : ''}>Irmão/Irmã</option>
                        <option value="friend" ${currentRelationship === 'friend' ? 'selected' : ''}>Amigo</option>
